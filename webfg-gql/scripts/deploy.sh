...
QA_SCHEMA_VERSION=$(node -p "require('./package.json').config.qa_schema") # Corrected path
PROD_SCHEMA_VERSION=$(node -p "require('./package.json').config.prod_schema") # Corrected path
