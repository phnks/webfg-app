...
jest.mock('react-router-dom', () => ({
  ...
  useParams: () => ({ thoughtId: 'test-thought-id' }),
...
jest.mock('../../../components/forms/ThoughtForm', () => {
  return function MockThoughtForm({ onClose, onSuccess, thought, isEditing }) {
    return (
      <div data-testid="thought-form">
        <span>Thought Form - Editing: {isEditing ? 'true' : 'false'}</span>
        <button onClick={() => onClose()}>Cancel</button>
        <button onClick={() => onSuccess('test-thought-id')}>Save</button>
      </div>
    );
  };
});
...
const { useSelectedCharacter } = require('../../../context/SelectedCharacterContext');
...
const ThoughtViewWrapper = ({ children }) => (
  <BrowserRouter>
    {children}
  </BrowserRouter>
);
...
const renderThoughtViewWithData = async (thoughtData = mockThought, selectedChar = mockSelectedCharacter) => {
  useQuery.mockImplementation((query, options) => {
    const mockReturnValue = {
      ...
      error: null,
      refetch: mockRefetch,
      data: thoughtData ? { getThought: thoughtData } : null
    };
  ...
  useMutation.mockImplementation((mutation) => {
    if (mutation === 'DELETE_THOUGHT') {
      return [mockDeleteThought, { loading: false }];
    }
    if (mutation === 'ADD_THOUGHT_TO_CHARACTER_MIND') {
      return [mockAddThoughtToCharacterMind, { loading: false }];
    }
...
describe('ThoughtView Component', () => {
  ...
  describe('Loading and Error States', () => {
    test('renders loading state', () => {
      ...
      useSelectedCharacter.mockReturnValue({ selectedCharacter: null });
  ...
  describe('Edit Mode', () => {
    test('shows edit form when startInEditMode is true', async () => {
      ...
      render(
        <ThoughtViewWrapper>
          <ThoughtView startInEditMode={true} />
        </ThoughtViewWrapper>
      );
    ...
    test('shows confirmation dialog and deletes on confirmation', async () => {
      ...
      await waitFor(() => {
        expect(mockDeleteThought).toHaveBeenCalledWith({
          variables: { thoughtId: 'test-thought-id' }
        });
  ...
  describe('Add to Character Mind', () => {
    test('adds thought to character mind successfully', async () => {
      mockAddThoughtToCharacterMind.mockResolvedValue({
        data: { addThoughtToCharacterMind: { success: true } }
      });
    ...
    test('handles graphQL error message', async () => {
      mockAddThoughtToCharacterMind.mockRejectedValue({
        message: 'Some message', // Need to have a message property to avoid TypeError
  ...
  describe('CSS Classes', () => {
    test('applies correct CSS classes', async () => {
      const { container } = await renderThoughtViewWithData();
