...
const mockGroupedAttributes = {
  speed: 12.5,
  // Example grouped with equipment
  complexity: 6.25,
  seeing: 6.5
};
const mockReadyGroupedAttributes = {
  speed: 11.8,
  // Example grouped with equipment + ready
  complexity: 5.8,
  hearing: 4.5,
  light: 3.25,
  noise: 2.25,
...
const renderComponent = (props = {}) => {
  ...
  const result = render(/*#__PURE__*/React.createElement(MockedProvider, {
    mocks: []
  }, /*#__PURE__*/React.createElement(CharacterAttributesBackend, defaultProps)));
...
describe('CharacterAttributesBackend', () => {
  ...
  describe('Ready Grouped Attributes Fallback', () => {
    ...
    it('should calculate fallback when backend data is missing specific attributes', () => {
      ...
      renderComponent({
        readyGroupedAttributes: partialReadyGrouped
      });
      const toggle = screen.getByRole('checkbox');
      fireEvent.click(toggle);
    ...
    it('should fix backend bug where specific attributes show 0 in ready mode', () => {
      ...
      const buggyBackendData = {
        ...
        // Correct value
        complexity: 0,
        // BUG: Should be grouped with ready objects
        hearing: 0,
        // BUG: Should be grouped with ready objects  
        light: 0,
        // BUG: Should be grouped with ready objects
      ...
      renderComponent({
        readyGroupedAttributes: buggyBackendData
      });
      const toggle = screen.getByRole('checkbox');
      fireEvent.click(toggle);
  ...
  describe('Grouped Value Display Logic', () => {
    ...
    it('should use correct formula (simple addition) in ready mode breakdown modal', () => {
      ...
      renderComponent({
        ...
        readyGroupedAttributes: {
          armour: 30
        } // Expected grouped result: 10 + 20 = 30
      });
      const toggle = screen.getByRole('checkbox');
      fireEvent.click(toggle);
