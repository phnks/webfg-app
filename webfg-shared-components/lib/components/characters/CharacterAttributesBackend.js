...
const CharacterAttributesBackend = ({
  character,
  // Full character object with all attributes
  groupedAttributes,
  // Equipment grouped attributes from backend
  ...
  } = useQuery(GET_CHARACTER_ATTRIBUTE_BREAKDOWN, {
    variables: {
  ...
  const generateFallbackBreakdown = attributeKey => {
    ...
    const relevantConditions = character.conditions.filter(c => c.conditionTarget && c.conditionTarget.toLowerCase() === attributeKey.toLowerCase());
  ...
  const generateReadyFallbackBreakdown = attributeKey => {
    ...
    if (character?.equipment?.length > 0) {
      ...
      inventoryItems.filter(invItem => invItem.inventoryLocation === 'EQUIPMENT').forEach(invItem => {
    ...
    if (character?.ready?.length > 0) {
      ...
      inventoryItems.filter(invItem => invItem.inventoryLocation === 'READY').forEach(invItem => {
  ...
  const effectiveReadyGroupedAttributes = useMemo(() => {
    if (readyGroupedAttributes) {
      ...
      const problematicAttributes = Object.values(ATTRIBUTE_GROUPS).flat().filter(attrName => {
        ...
        const isSuspiciouslyLow = backendValue !== undefined && backendValue < charBaseValue && (character?.ready?.length > 0 || character?.equipment?.length > 0);
  ...
  const renderAttributeForView = (attributeName, attribute, displayName) => {
    ...
    const hasConditionForThisAttribute = hasConditions && character.conditions.some(c => c.conditionTarget && c.conditionTarget.toLowerCase() === attributeName.toLowerCase());
    ...
    readyGroupedValue !== undefined && readyGroupedValue !== null && (hasReady || hasConditionForThisAttribute || isDisplayDifferent) || hasConditionForThisAttribute && effectiveReadyGroupedAttributes && effectiveReadyGroupedAttributes[attributeName] !== undefined ||
    // Also show if equipment mode would show it (to maintain consistency)
    equipmentGroupedValue !== undefined && equipmentGroupedValue !== null && (hasEquipment || hasConditionForThisAttribute) || hasConditionForThisAttribute && effectiveGroupedAttributes && effectiveGroupedAttributes[attributeName] !== undefined :
    // For equipment mode: use existing logic
    equipmentGroupedValue !== undefined && equipmentGroupedValue !== null && (hasEquipment || hasConditionForThisAttribute || isDisplayDifferent) || hasConditionForThisAttribute && effectiveGroupedAttributes && effectiveGroupedAttributes[attributeName] !== undefined;
    ...
    }, /*#__PURE__*/React.createElement("label", null, displayName), /*#__PURE__*/React.createElement("span", null, originalValue, /*#__PURE__*/React.createElement("span", {
    ...
    }, character?.[attributeName]?.attribute?.isGrouped !== false ? '☑️' : '❌'), shouldShowGroupedValue && /*#__PURE__*/React.createElement("span", {
    ...
    }, ' → ', displayGroupedValue !== undefined && !isNaN(Number(displayGroupedValue)) ? Math.round(Number(displayGroupedValue)) : originalValue, ((showReadyAttributes ? hasReady : hasEquipment) || hasConditions) && /*#__PURE__*/React.createElement("button", {
      className: "info-icon",
      onClick: e => {
        e.stopPropagation();
  ...
  if (!character) {
    return /*#__PURE__*/React.createElement("div", {
      className: "section character-attributes"
    }, /*#__PURE__*/React.createElement("h3", null, "Attributes"), /*#__PURE__*/React.createElement("p", null, "No character data available."));
  }
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "section character-attributes"
  }, /*#__PURE__*/React.createElement("div", {
    className: "attributes-header"
  }, /*#__PURE__*/React.createElement("h3", null, "Attributes"), (character?.ready?.length > 0 || readyGroupedAttributes) && /*#__PURE__*/React.createElement("div", {
    className: "equipment-ready-toggle"
  ...
  }, showReadyAttributes ? 'ready' : 'equipment')))), /*#__PURE__*/React.createElement(AttributeGroups, {
    ...
    breakdown: showReadyAttributes ? generateReadyFallbackBreakdown(selectedAttribute) : breakdownData?.getCharacter?.attributeBreakdown || generateFallbackBreakdown(selectedAttribute),
