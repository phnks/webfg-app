...
const Timeline = ({
  ...
  const getFormattedDescription = event => {
    ...
    switch (event.type) {
      ...
      // Assuming backend provides full description like "Character started ActionName"
      case 'ACTION_COMPLETED':
        return event.description;
      // Assuming backend provides full description
      // CHARACTER_JOINED, CHARACTER_MOVED, OBJECT_ADDED, OBJECT_MOVED should already have names/scaled coords
      default:
        return event.description;
      // Use the enriched description from the history event
    }
  };
  ...
  const timelineEvents = React.useMemo(() => {
    ...
    return [...history].filter(event => event != null && event.time != null) // Add filter for null/undefined
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    }, event.time && typeof event.time === 'number' ? event.time.toFixed(1) : '0.0', "s"), /*#__PURE__*/React.createElement("div", {
      className: "event-content",
      onClick: () => event.characterId && onSelectCharacter(event.characterId)
    }, /*#__PURE__*/React.createElement("div", {
      className: "event-description"
    }, getFormattedDescription(event)), event.action && /*#__PURE__*/React.createElement("div", {
      className: "event-action-duration"
  ...
  }, currentTime && typeof currentTime === 'number' ? currentTime.toFixed(1) : '0.0', "s"))));
};
