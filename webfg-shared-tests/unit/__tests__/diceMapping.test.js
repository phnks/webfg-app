import { ATTRIBUTE_DICE_MAP, getDiceForAttribute, attributeUsesDice, getDiceRange, calculateAttributeModifier, formatDiceRoll, getAttributeRange } from '../../utils/diceMapping';
describe('diceMapping utility', () => {
  ...
  describe('getDiceRange', () => {
    test('returns correct range for standard dice', () => {
      expect(getDiceRange('d4')).toEqual({
        min: 1,
        max: 4
      });
      expect(getDiceRange('d6')).toEqual({
        min: 1,
        max: 6
      });
      expect(getDiceRange('d8')).toEqual({
        min: 1,
        max: 8
      });
      expect(getDiceRange('d10')).toEqual({
        min: 1,
        max: 10
      });
      expect(getDiceRange('d12')).toEqual({
        min: 1,
        max: 12
      });
      expect(getDiceRange('d20')).toEqual({
        min: 1,
        max: 20
      });
      expect(getDiceRange('d100')).toEqual({
        min: 1,
        max: 100
      });
    });
    test('returns zero range for null/undefined input', () => {
      expect(getDiceRange(null)).toEqual({
        min: 0,
        max: 0
      });
      expect(getDiceRange(undefined)).toEqual({
        min: 0,
        max: 0
      });
      expect(getDiceRange('')).toEqual({
        min: 0,
        max: 0
      });
    });
    test('handles unusual dice types', () => {
      expect(getDiceRange('d2')).toEqual({
        min: 1,
        max: 2
      });
      expect(getDiceRange('d3')).toEqual({
        min: 1,
        max: 3
      });
      expect(getDiceRange('d30')).toEqual({
        min: 1,
        max: 30
      });
    });
  ...
  describe('getAttributeRange', () => {
    test('calculates range for dice-based attributes', () => {
      expect(getAttributeRange('SPEED', 5)).toEqual({
        min: 6,
        max: 9
      }); // d4: 1-4, +5 = 6-9
      expect(getAttributeRange('STRENGTH', 3)).toEqual({
        min: 4,
        max: 11
      }); // d8: 1-8, +3 = 4-11
      expect(getAttributeRange('DEXTERITY', 0)).toEqual({
        min: 1,
        max: 6
      }); // d6: 1-6, +0 = 1-6
      expect(getAttributeRange('AGILITY', 10)).toEqual({
        min: 11,
        max: 20
      }); // d10: 1-10, +10 = 11-20
      expect(getAttributeRange('CHARISMA', 2)).toEqual({
        min: 3,
        max: 102
      }); // d100: 1-100, +2 = 3-102
      expect(getAttributeRange('INTELLIGENCE', 8)).toEqual({
        min: 9,
        max: 28
      }); // d20: 1-20, +8 = 9-28
      expect(getAttributeRange('RESOLVE', 4)).toEqual({
        min: 5,
        max: 16
      }); // d12: 1-12, +4 = 5-16
    });
    test('returns static value for static attributes', () => {
      expect(getAttributeRange('WEIGHT', 10)).toEqual({
        min: 10,
        max: 10
      });
      expect(getAttributeRange('SIZE', 5)).toEqual({
        min: 5,
        max: 5
      });
      expect(getAttributeRange('LETHALITY', 0)).toEqual({
        min: 0,
        max: 0
      });
      expect(getAttributeRange('ARMOUR', 15)).toEqual({
        min: 15,
        max: 15
      });
    });
    test('handles unknown attributes as static', () => {
      expect(getAttributeRange('UNKNOWN', 7)).toEqual({
        min: 7,
        max: 7
      });
      expect(getAttributeRange('FOO', 12)).toEqual({
        min: 12,
        max: 12
      });
    });
    test('handles negative modifiers', () => {
      expect(getAttributeRange('STRENGTH', -2)).toEqual({
        min: -1,
        max: 6
      }); // d8: 1-8, -2 = -1-6
      expect(getAttributeRange('WEIGHT', -5)).toEqual({
        min: -5,
        max: -5
      });
    });
    test('handles zero modifier', () => {
      expect(getAttributeRange('SPEED', 0)).toEqual({
        min: 1,
        max: 4
      });
      expect(getAttributeRange('WEIGHT', 0)).toEqual({
        min: 0,
        max: 0
      });
    });
